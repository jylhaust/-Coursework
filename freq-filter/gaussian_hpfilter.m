...
for i = 1 : 3
     H( :, :, i) = hpfilter('gaussian', PQ(1), PQ(2), D1(i)); % ideal gaussian btw
     g(:, :, i) = dftfilt(f, H(:, :, i));
end
...
subplot(221); imshow(f, []); title('Original Pic');
subplot(222); imshow(g(:, :, 1), []); title('gaussian D1=15');
subplot(223); imshow(g(:, :, 2), []); title('gaussian D1=30');
subplot(224); imshow(g(:, :, 3), []); title('gaussian D1=80');