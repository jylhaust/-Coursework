...
for i = 1 : 3
     H( :, :, i) = lpfilter('ideal', PQ(1), PQ(2), D0(i)); % ideal gaussian btw
     g(:, :, i) = dftfilt(f, H(:, :, i));
end
subplot(221); imshow(f, []); title('Original Pic');
subplot(222); imshow(g(:, :, 1), []); title('ideal D0=30');
subplot(223); imshow(g(:, :, 2), []); title('ideal D0=50');
subplot(224); imshow(g(:, :, 3), []); title('ideal D0=200');
% subplot(221); imshow(f, []); title('Original Pic');
